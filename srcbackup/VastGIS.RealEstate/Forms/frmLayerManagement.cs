...
namespace VastGIS.Plugins.RealEstate.Forms
{
    ...
    {
        private BindingList<VgObjectclass> _vgObjectclasseses;
        private List<VgObjectclass> _tclbList;
        ...
        public frmLayerManagement(IAppContext context) : base(context)
        {
            ...
            _tclbList = ((IRealEstateContext)_context).RealEstateDatabase.SystemService.GetObjectclasses(true).Where(c => c.Dxlx == 0).OrderByDescending(c=>c.ID).ToList();
        ...
        private void LoadFromMap()
        {
            ...
            {
                VgObjectclass oneClass = _vgObjectclasseses.FirstOrDefault(c => c.Zwmc == legendLayer.Name);
                if (oneClass == null)
                    continue;
        ...
        private void UpdateDB()
        {
            ...
            service.SaveVgObjectclasses(new List<VgObjectclass>(_vgObjectclasseses));
        }
        ...
        {
            VgObjectclass oneClass = cmbLayerType.SelectedItem as VgObjectclass;
            if (oneClass == null)
                return;
            _vgObjectclasseses = new BindingList<VgObjectclass>(oneClass.SubClasses.OrderByDescending(c => c.ID).ToList());
        ...
        private void CheckSnapableOnCheckedChanged(object sender, EventArgs eventArgs)
        {
            ...
            foreach (VgObjectclass oneClass in _vgObjectclasseses)
            {
