        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            if (ucSelectLayer1.SelectedClass.Txlx != 1)
            {
                ...
                SearchFeature feature =
                    new SearchFeature() { Wkt = geometry.ExportToWkt(), TableName = ucSelectLayer1.SelectedClass.Mc };
                long id = _database.SystemService.SaveSearchFeature(feature);
                if (id > 0)
                {
            ...
            {
                List<SearchFeature> inFeatures = new List<SearchFeature>();
                for (int i = 0; i < geometry.Points.Count; i++)
                {
                    SearchFeature feature =
                        new SearchFeature() { Wkt = string.Format("POINT({0} {1})", geometry.Points[i].X, geometry.Points[i].Y), TableName = ucSelectLayer1.SelectedClass.Mc };
                    inFeatures.Add(feature);
                }
                List<long> newIds = _database.SystemService.SaveSearchFeatures(inFeatures);
                ucCoordinateList1.ClearList();
            }
        }
