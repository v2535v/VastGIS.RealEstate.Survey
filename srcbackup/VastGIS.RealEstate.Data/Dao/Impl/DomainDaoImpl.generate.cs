...
namespace VastGIS.RealEstate.Data.Dao.Impl
{
    ...
    public partial class DomainDaoImpl:SQLiteDao,DomainDao
    {
        ...
        {
            bool retVal= vgDictionary.Save(this);
            if(retVal)
            {
        ...
        public void SaveVgDictionaries(List<VgDictionary> vgDictionaries)
        {
            ...
            foreach(var rec in vgDictionaries)
            {
                rec.Save(this);
            }
            tran.Commit();
            tran.Dispose();
            List<long> ids=vgDictionaries.Select(a => a.ID).ToList(); 
        ...
        {
            bool retVal= vgDictoryname.Save(this);
            if(retVal)
            {
