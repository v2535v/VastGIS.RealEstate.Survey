...
namespace VastGIS.Plugins.RealEstate.Viewer
{
    public partial class ucImageViewer : UserControl
    {
        ...
        public void LoadImages(List<string> files, string oneFile)
        {
            if (_imageFiles == null)
            {
        ...
        public int CurrentImageIndex
        {
            ...
            set
            {
                ...
                if (_imageFiles == null)
                {
                    btnPrevStep.Enabled = btnNextStep.Enabled = false;
                    return;
                }
                if (_currentImageIndex > _imageFiles.Count - 1) return;
                this.ImagePath = _imageFiles[value];
                btnPrevStep.Enabled = _currentImageIndex > 0;
                btnNextStep.Enabled = _currentImageIndex < (_imageFiles.Count - 1);
            }
        }
        ...
        private void ucImageViewer_Load(object sender, EventArgs e)
        {
        ...
        private void ucImageViewer_Resize(object sender, EventArgs e)
        {
            InitControl();
        ...
        private void pbFull_Paint(object sender, PaintEventArgs e)
        {
            ...
            {
                // Yes I can!
                drawEngine.g.FillRectangle(new SolidBrush(pbFull.BackColor), e.ClipRectangle.X, e.ClipRectangle.Y,
        ...
        private void ucImageViewer_MouseWheel(object sender, MouseEventArgs e)
        {
        ...
        {
            drawing.JumpToOrigin(e.X + (drawing.BoundingBox.X - (drawing.BoundingBox.X * 2)),
                e.Y + (drawing.BoundingBox.Y - (drawing.BoundingBox.Y * 2)), pbFull.Width, pbFull.Height);
            UpdatePanels(true);
        }
        ...
        private void ucImageViewer_Click(object sender, EventArgs e)
        {
            FocusOnMe();
        }
            ...
            try
            {
                ...
                for (int f = 0; f < FileList.Length; f++)
                {
                    ...
                    if (System.IO.File.Exists(FileList[f]))
                    {
                        ...
                        if (ext == ".jpg" || ext == ".jpeg" || ext == ".gif" || ext == ".wmf" || ext == ".emf" ||
                            ext == ".bmp" || ext == ".png" || ext == ".tif" || ext == ".tiff")
                        {
                            try
                            {
        ...
        private void btnMode_Click(object sender, EventArgs e)
        {
            ...
            {
                selectMode = true;
                this.btnMode.Image = Properties.Resources.img_hand;
            }
            else
            {
                selectMode = false;
                this.btnMode.Image = Properties.Resources.img_select;
            }
        }
        ...
        {
            if (_currentImageIndex >= _imageFiles.Count - 1) return;
            int newValue = _currentImageIndex + 1;
