...
namespace VastGIS.Plugins.RealEstate.Forms
{
    public partial class frmUnionGeometry:Form,IEditForm
    {
        ...
        public frmUnionGeometry(IAppContext context, RealEstateEditor plugin)
        {
            ...
            {
                if (_plugin.Config.EditingClass.Txlx != 3)
                {
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            IReFeature mainFeature = ucFeatureLists1.GetCurrentSelectedAndCheckedFeature();
            if (mainFeature == null)
            {
           ...
           bool retVal=_database.SystemService.Save(mainFeature);
            if (!retVal ||mainFeature.ID<1)
            {
                ucFeatureLists1.Redraw();
                MessageService.Current.Warn("图形未能正确保存!");
                return;
            }
            List<IEntity> deletFeatures = ucFeatureLists1.GetSelectedFeatures().Where(c=>c.ID != mainFeature.ID).ToList<IEntity>();
            long[] ids = deletFeatures.Select(c => c.ID).ToArray();
            _database.SystemService.Delete(deletFeatures);
            ucFeatureLists1.ClearList();
            ucFeatureLists1.ClearDrawing();
        }
    }
