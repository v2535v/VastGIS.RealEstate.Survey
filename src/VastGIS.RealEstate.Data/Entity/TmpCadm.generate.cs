...
{
    public partial class TmpCadm : INotifyPropertyChanging, INotifyPropertyChanged,IDatabaseFeature
    {		
		#region 表结构		
        public const string TABLE_NAME = "TmpCadm";	
        public const string LAYER_NAME = "TmpCadm";	
		...
		#region SQL语句
		private const string SQL_QUERY_TMPCADM ="SELECT  Id,EntityType,Handle,FileName,DatabaseId,FLAGS,XGR,XGSJ,AsText(geometry) as Wkt FROM TmpCadm WHERE [FLAGS]<3";
        
		private const string SQL_INSERT_TMPCADM = "INSERT INTO [TmpCadm] (EntityType, Handle, FileName, DatabaseId, FLAGS, XGR, XGSJ, geometry) VALUES ( @EntityType, @Handle, @FileName, @DatabaseId, @FLAGS, @XGR, @XGSJ, GeomFromText(@geometry,@SRID));" + " SELECT last_insert_rowid();";
		
		private const string SQL_UPDATE_TMPCADM = "UPDATE [TmpCadm] SET EntityType = @EntityType, Handle = @Handle, FileName = @FileName, DatabaseId = @DatabaseId, FLAGS = @FLAGS, XGR = @XGR, XGSJ = @XGSJ,  geometry = GeomFromText(@geometry,@SRID) WHERE Id = @Id";
		
		private const string SQL_DELETE_TMPCADM = "DELETE FROM [TmpCadm] WHERE  Id = @Id ";
		
        private const string SQL_DELETE_FLAG_TMPCADM = "UPDATE [TmpCadm] Set Flags=3,XGR = @XGR, XGSJ = @XGSJ WHERE WHERE  Id = @Id ";
        #endregion
       ...
       public IGeometry Geometry
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_INSERT_TMPCADM,dao.Connection))
            {
                this.databaseid=0;
                this.flags=1;
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_UPDATE_TMPCADM,dao.Connection))
            {
        ...
        public bool Delete(ISQLiteService dao)
        {
            if(this.databaseid==0)
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_TMPCADM,dao.Connection))
                {                   
    				command.Parameters.AddWithValue(PARAM_ID, this.ID);
                    return (command.ExecuteNonQuery() == 1);
                }
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_FLAG_TMPCADM,dao.Connection))
                {
