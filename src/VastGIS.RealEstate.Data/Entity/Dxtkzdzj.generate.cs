...
{
    public partial class Dxtkzdzj : INotifyPropertyChanging, INotifyPropertyChanged,IBasemapText,IDatabaseFeature,IYsdmEntity
    {		
		#region 表结构		
        public const string TABLE_NAME = "DXTKZDZJ";	
        public const string LAYER_NAME = "控制点注记";	
		...
		#region SQL语句
		private const string SQL_QUERY_DXTKZDZJ ="SELECT  Id,WBNR,TC,CASSDM,FH,FHDX,XZJD,YSDM,DatabaseId,FLAGS,XGR,XGSJ,AsText(geometry) as Wkt FROM DXTKZDZJ WHERE [FLAGS]<3";
        
		private const string SQL_INSERT_DXTKZDZJ = "INSERT INTO [DXTKZDZJ] (WBNR, TC, CASSDM, FH, FHDX, XZJD, YSDM, DatabaseId, FLAGS, XGR, XGSJ, geometry) VALUES ( @WBNR, @TC, @CASSDM, @FH, @FHDX, @XZJD, @YSDM, @DatabaseId, @FLAGS, @XGR, @XGSJ, GeomFromText(@geometry,@SRID));" + " SELECT last_insert_rowid();";
		
		private const string SQL_UPDATE_DXTKZDZJ = "UPDATE [DXTKZDZJ] SET WBNR = @WBNR, TC = @TC, CASSDM = @CASSDM, FH = @FH, FHDX = @FHDX, XZJD = @XZJD, YSDM = @YSDM, DatabaseId = @DatabaseId, FLAGS = @FLAGS, XGR = @XGR, XGSJ = @XGSJ,  geometry = GeomFromText(@geometry,@SRID) WHERE Id = @Id";
		
		private const string SQL_DELETE_DXTKZDZJ = "DELETE FROM [DXTKZDZJ] WHERE  Id = @Id ";
		
        private const string SQL_DELETE_FLAG_DXTKZDZJ = "UPDATE [DXTKZDZJ] Set Flags=3,XGR = @XGR, XGSJ = @XGSJ WHERE WHERE  Id = @Id ";
        #endregion
        ...
        public string SimpleLabelString
        {
            get
            {
                return string.Format("{0} {1} ({2})","DXTKZDZJ",this.id,this.ysdm);
            }
        }
        ...
        public string FullLabelString
        {
            get
            {
                return string.Format("{0} {1} ({2})","控制点注记",this.id,this.ysdm);
            }
        }        
        #endregion
       ...
       public IGeometry Geometry
        ...
        {
            Dxtkzdzj record = obj as Dxtkzdzj;           
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_INSERT_DXTKZDZJ,dao.Connection))
            {
                this.databaseid=0;
                this.flags=1;
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_UPDATE_DXTKZDZJ,dao.Connection))
            {
        ...
        public bool Delete(ISQLiteService dao)
        {
            if(this.databaseid==0)
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_DXTKZDZJ,dao.Connection))
                {                   
    				command.Parameters.AddWithValue(PARAM_ID, this.ID);
                    return (command.ExecuteNonQuery() == 1);
                }
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_FLAG_DXTKZDZJ,dao.Connection))
                {
