...
{
    public partial class Jzdzj : INotifyPropertyChanging, INotifyPropertyChanged,IDatabaseFeature,IYsdmEntity
    {		
		#region 表结构		
		...
		#region SQL语句
		private const string SQL_QUERY_JZDZJ ="SELECT  Id,GLYSBH,YSDM,ZJNR,ZT,YS,BS,XZ,XHX,KD,GD,ZJDZXJXZB,ZJDZXJYZB,ZJFX,DatabaseId,FLAGS,XGR,XGSJ,AsText(geometry) as Wkt FROM JZDZJ WHERE [FLAGS]<3";
        ...
        private const string SQL_DELETE_FLAG_JZDZJ = "UPDATE [JZDZJ] Set Flags=3,XGR = @XGR, XGSJ = @XGSJ WHERE WHERE  Id = @Id ";
        #endregion
        ...
        public string FullLabelString
        {
            get
            {
                return string.Format("{0} {1} ({2})","界址点注记",this.id,this.ysdm);
            }
        }        
        #endregion
