...
{
    public partial class Yhydzb : INotifyPropertyChanging, INotifyPropertyChanged,IDatabaseEntity
    {		
		#region 表结构		
        public const string TABLE_NAME = "YHYDZB";	
        public const string LAYER_NAME = "用海用岛坐标";	
        ...
        public const string PARAM_BW = "@BW";	
		...
		#region SQL语句
		private const string SQL_QUERY_YHYDZB ="SELECT  Id,ZHHDDM,XH,BW,DJ,DatabaseId,FLAGS,XGR,XGSJ FROM YHYDZB WHERE [FLAGS]<3";
        
		private const string SQL_INSERT_YHYDZB = "INSERT INTO [YHYDZB] (ZHHDDM, XH, BW, DJ, DatabaseId, FLAGS, XGR, XGSJ) VALUES ( @ZHHDDM, @XH, @BW, @DJ, @DatabaseId, @FLAGS, @XGR, @XGSJ);" + " SELECT last_insert_rowid();";
		
		private const string SQL_UPDATE_YHYDZB = "UPDATE [YHYDZB] SET ZHHDDM = @ZHHDDM, XH = @XH, BW = @BW, DJ = @DJ, DatabaseId = @DatabaseId, FLAGS = @FLAGS, XGR = @XGR, XGSJ = @XGSJ WHERE Id = @Id";
		
		private const string SQL_DELETE_YHYDZB = "DELETE FROM [YHYDZB] WHERE  Id = @Id ";
		
        private const string SQL_DELETE_FLAG_YHYDZB = "UPDATE [YHYDZB] Set Flags=3,XGR = @XGR, XGSJ = @XGSJ WHERE WHERE  Id = @Id ";
        #endregion
        ...
        ///序号
		protected long? xh = default(long?);
        ///北纬
		protected double? bw = default(double?);
        ///东经
		protected double? dj = default(double?);
        ///原数据库内序号
        ...
        public string SimpleLabelString
        {
            get
            {
                return string.Format("{0} {1} ","YHYDZB",this.id);
            }
        }
        ...
        public string FullLabelString
        {
            get
            {
                return string.Format("{0} {1} ","用海用岛坐标",this.id);
            }
        }        
        #endregion
        ...
        ///北纬
        ///[Column(COL_BW, PARAM_BW )]
        public virtual double? Bw 
        {
            get { return this.bw; }
			set	{ 
                  if(this.bw != value)
                    {
                        this.OnPropertyChanging("Bw"); 
                        this.bw = value;                        
                        this.OnPropertyChanged("Bw");
                    }   
                }
        }	
       ...
       ///图形类型
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_INSERT_YHYDZB,dao.Connection))
            {
                this.databaseid=0;
                this.flags=1;
				...
				command.Parameters.AddWithValue(PARAM_BW, this.Bw);
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_UPDATE_YHYDZB,dao.Connection))
            {
        ...
        public bool Delete(ISQLiteService dao)
        {
            if(this.databaseid==0)
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_YHYDZB,dao.Connection))
                {                   
    				command.Parameters.AddWithValue(PARAM_ID, this.ID);
                    return (command.ExecuteNonQuery() == 1);
                }
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_FLAG_YHYDZB,dao.Connection))
                {
        ...
        public void  LoadFromReader(SQLiteDataReader reader)
        {
			...
			if (!reader.IsDBNull(1)) zhhddm = reader.GetString(1);
			if (!reader.IsDBNull(2)) xh = reader.GetInt64(2);
			if (!reader.IsDBNull(3)) bw = reader.GetDouble(3);
			if (!reader.IsDBNull(4)) dj = reader.GetDouble(4);
