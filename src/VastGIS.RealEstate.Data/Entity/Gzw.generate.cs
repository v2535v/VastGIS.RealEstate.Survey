...
{
    public partial class Gzw : INotifyPropertyChanging, INotifyPropertyChanged,ISurveyFeature,IYsdmEntity,IGlobalEntity
    {		
		#region 表结构		
        public const string TABLE_NAME = "GZW";	
        public const string LAYER_NAME = "构筑物";	
		...
		public const string COL_GZWMC = "GZWMC";
        ...
        public const string PARAM_GZWMC = "@GZWMC";	
		...
		#region SQL语句
		private const string SQL_QUERY_GZW ="SELECT  Id,YSDM,BDCDYH,ZDZHDM,GZWMC,ZL,MJDW,MJ,DAH,ZT,WX_DCY,WX_DCSJ,WX_CLY,WX_CLSJ,WX_ZTY,WX_ZTSJ,WX_ZJY,WX_ZJSJ,WX_WYDM,DatabaseId,FLAGS,XGR,XGSJ,AsText(geometry) as Wkt FROM GZW WHERE [FLAGS]<3";
        
		private const string SQL_INSERT_GZW = "INSERT INTO [GZW] (YSDM, BDCDYH, ZDZHDM, GZWMC, ZL, MJDW, MJ, DAH, ZT, WX_DCY, WX_DCSJ, WX_CLY, WX_CLSJ, WX_ZTY, WX_ZTSJ, WX_ZJY, WX_ZJSJ, WX_WYDM, DatabaseId, FLAGS, XGR, XGSJ, geometry) VALUES ( @YSDM, @BDCDYH, @ZDZHDM, @GZWMC, @ZL, @MJDW, @MJ, @DAH, @ZT, @WX_DCY, @WX_DCSJ, @WX_CLY, @WX_CLSJ, @WX_ZTY, @WX_ZTSJ, @WX_ZJY, @WX_ZJSJ, @WX_WYDM, @DatabaseId, @FLAGS, @XGR, @XGSJ, GeomFromText(@geometry,@SRID));" + " SELECT last_insert_rowid();";
		
		private const string SQL_UPDATE_GZW = "UPDATE [GZW] SET YSDM = @YSDM, BDCDYH = @BDCDYH, ZDZHDM = @ZDZHDM, GZWMC = @GZWMC, ZL = @ZL, MJDW = @MJDW, MJ = @MJ, DAH = @DAH, ZT = @ZT, WX_DCY = @WX_DCY, WX_DCSJ = @WX_DCSJ, WX_CLY = @WX_CLY, WX_CLSJ = @WX_CLSJ, WX_ZTY = @WX_ZTY, WX_ZTSJ = @WX_ZTSJ, WX_ZJY = @WX_ZJY, WX_ZJSJ = @WX_ZJSJ, WX_WYDM = @WX_WYDM, DatabaseId = @DatabaseId, FLAGS = @FLAGS, XGR = @XGR, XGSJ = @XGSJ,  geometry = GeomFromText(@geometry,@SRID) WHERE Id = @Id";
		
		private const string SQL_DELETE_GZW = "DELETE FROM [GZW] WHERE  Id = @Id ";
		
        private const string SQL_DELETE_FLAG_GZW = "UPDATE [GZW] Set Flags=3,XGR = @XGR, XGSJ = @XGSJ WHERE WHERE  Id = @Id ";
        #endregion
        ...
        ///构筑物名称
		protected string gzwmc = default(string);
        ///坐落
        ...
        public string SimpleLabelString
        {
            get
            {
                return string.Format("{0} {1} ({2})","GZW",this.id,this.ysdm);
            }
        }
        ...
        public string FullLabelString
        {
            get
            {
                return string.Format("{0} {1} ({2})","构筑物",this.id,this.ysdm);
            }
        }        
        #endregion
        ...
        ///构筑物名称
        ///[Column(COL_GZWMC, PARAM_GZWMC )]
        public virtual string Gzwmc 
        {
            get { return this.gzwmc; }
			set	{ 
                  if(this.gzwmc != value)
                    {
                        this.OnPropertyChanging("Gzwmc"); 
                        this.gzwmc = value;                        
                        this.OnPropertyChanged("Gzwmc");
                    }   
                }
        }	
       ...
       public IGeometry Geometry
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_INSERT_GZW,dao.Connection))
            {
                this.databaseid=0;
                this.flags=1;
				...
				command.Parameters.AddWithValue(PARAM_GZWMC, this.Gzwmc);
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_UPDATE_GZW,dao.Connection))
            {
        ...
        public bool Delete(ISQLiteService dao)
        {
            if(this.databaseid==0)
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_GZW,dao.Connection))
                {                   
    				command.Parameters.AddWithValue(PARAM_ID, this.ID);
                    return (command.ExecuteNonQuery() == 1);
                }
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_FLAG_GZW,dao.Connection))
                {
        ...
        public void  LoadFromReader(SQLiteDataReader reader)
        {
			...
			if (!reader.IsDBNull(4)) gzwmc = reader.GetString(4);
