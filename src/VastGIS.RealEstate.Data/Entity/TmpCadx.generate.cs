...
namespace VastGIS.RealEstate.Data.Entity
{
    ...
    public partial class TmpCadx:INotifyPropertyChanging, INotifyPropertyChanged
    {
        #region 表结构
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_INSERT_TMPCADX,connection))
            {	
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_UPDATE_TMPCADX,connection))
            {							
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_TMPCADX,connection))
            {							
				command.Parameters.AddWithValue(PARAM_ID, this.ID);
                return (command.ExecuteNonQuery() == 1);
            }
