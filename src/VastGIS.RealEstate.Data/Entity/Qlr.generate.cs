...
{
    public partial class Qlr : INotifyPropertyChanging, INotifyPropertyChanged,IDatabaseEntity,IYsdmEntity
    {		
		#region 表结构		
		...
		#region SQL语句
		private const string SQL_QUERY_QLR ="SELECT  Id,YSDM,BDCDYH,SXH,QLRMC,BDCQZH,QZYSXLH,SFCZR,ZJZL,ZJH,FZJG,SSHY,GJ,HJSZSS,XB,DH,DZ,YB,GZDW,DZYJ,QLRLX,QLBL,GYFS,GYQK,BZ,DatabaseId,FLAGS,XGR,XGSJ FROM QLR WHERE [FLAGS]<3";
        
		private const string SQL_INSERT_QLR = "INSERT INTO [QLR] (YSDM, BDCDYH, SXH, QLRMC, BDCQZH, QZYSXLH, SFCZR, ZJZL, ZJH, FZJG, SSHY, GJ, HJSZSS, XB, DH, DZ, YB, GZDW, DZYJ, QLRLX, QLBL, GYFS, GYQK, BZ, DatabaseId, FLAGS, XGR, XGSJ) VALUES ( @YSDM, @BDCDYH, @SXH, @QLRMC, @BDCQZH, @QZYSXLH, @SFCZR, @ZJZL, @ZJH, @FZJG, @SSHY, @GJ, @HJSZSS, @XB, @DH, @DZ, @YB, @GZDW, @DZYJ, @QLRLX, @QLBL, @GYFS, @GYQK, @BZ, @DatabaseId, @FLAGS, @XGR, @XGSJ);" + " SELECT last_insert_rowid();";
		
		private const string SQL_UPDATE_QLR = "UPDATE [QLR] SET YSDM = @YSDM, BDCDYH = @BDCDYH, SXH = @SXH, QLRMC = @QLRMC, BDCQZH = @BDCQZH, QZYSXLH = @QZYSXLH, SFCZR = @SFCZR, ZJZL = @ZJZL, ZJH = @ZJH, FZJG = @FZJG, SSHY = @SSHY, GJ = @GJ, HJSZSS = @HJSZSS, XB = @XB, DH = @DH, DZ = @DZ, YB = @YB, GZDW = @GZDW, DZYJ = @DZYJ, QLRLX = @QLRLX, QLBL = @QLBL, GYFS = @GYFS, GYQK = @GYQK, BZ = @BZ, DatabaseId = @DatabaseId, FLAGS = @FLAGS, XGR = @XGR, XGSJ = @XGSJ WHERE Id = @Id";
