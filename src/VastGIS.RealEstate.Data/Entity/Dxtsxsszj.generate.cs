...
{
    public partial class Dxtsxsszj : INotifyPropertyChanging, INotifyPropertyChanged,IBasemapText,IDatabaseFeature,IYsdmEntity
    {		
		#region 表结构		
		...
		#region SQL语句
		private const string SQL_QUERY_DXTSXSSZJ ="SELECT  Id,WBNR,TC,CASSDM,FH,FHDX,XZJD,YSDM,DatabaseId,FLAGS,XGR,XGSJ,AsText(geometry) as Wkt FROM DXTSXSSZJ WHERE [FLAGS]<3";
        
		private const string SQL_INSERT_DXTSXSSZJ = "INSERT INTO [DXTSXSSZJ] (WBNR, TC, CASSDM, FH, FHDX, XZJD, YSDM, DatabaseId, FLAGS, XGR, XGSJ, geometry) VALUES ( @WBNR, @TC, @CASSDM, @FH, @FHDX, @XZJD, @YSDM, @DatabaseId, @FLAGS, @XGR, @XGSJ, GeomFromText(@geometry,@SRID));" + " SELECT last_insert_rowid();";
		
		private const string SQL_UPDATE_DXTSXSSZJ = "UPDATE [DXTSXSSZJ] SET WBNR = @WBNR, TC = @TC, CASSDM = @CASSDM, FH = @FH, FHDX = @FHDX, XZJD = @XZJD, YSDM = @YSDM, DatabaseId = @DatabaseId, FLAGS = @FLAGS, XGR = @XGR, XGSJ = @XGSJ,  geometry = GeomFromText(@geometry,@SRID) WHERE Id = @Id";
		
		private const string SQL_DELETE_DXTSXSSZJ = "DELETE FROM [DXTSXSSZJ] WHERE  Id = @Id ";
		
        private const string SQL_DELETE_FLAG_DXTSXSSZJ = "UPDATE [DXTSXSSZJ] Set Flags=3,XGR = @XGR, XGSJ = @XGSJ WHERE WHERE  Id = @Id ";
        #endregion
        ...
        public string FullLabelString
        {
            get
            {
                return string.Format("{0} {1} ({2})","水系设施注记",this.id,this.ysdm);
            }
        }        
        #endregion
       ...
       public IGeometry Geometry
        ...
        {
            Dxtsxsszj record = obj as Dxtsxsszj;           
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_INSERT_DXTSXSSZJ,dao.Connection))
            {
                this.databaseid=0;
                this.flags=1;
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_UPDATE_DXTSXSSZJ,dao.Connection))
            {
        ...
        public bool Delete(ISQLiteService dao)
        {
            if(this.databaseid==0)
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_DXTSXSSZJ,dao.Connection))
                {                   
    				command.Parameters.AddWithValue(PARAM_ID, this.ID);
                    return (command.ExecuteNonQuery() == 1);
                }
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_FLAG_DXTSXSSZJ,dao.Connection))
                {
        ...
        public IEntity Copy()
        {
            Dxtsxsszj target=new Dxtsxsszj();
            target.ID=0;
