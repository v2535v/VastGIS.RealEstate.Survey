...
{
    public partial class Dxtdlsszj : INotifyPropertyChanging, INotifyPropertyChanged,IBasemapText,IDatabaseFeature,IYsdmEntity
    {		
		#region 表结构		
        public const string TABLE_NAME = "DXTDLSSZJ";	
        public const string LAYER_NAME = "道路设施注记";	
		...
		#region SQL语句
		private const string SQL_QUERY_DXTDLSSZJ ="SELECT  Id,WBNR,TC,CASSDM,FH,FHDX,XZJD,YSDM,DatabaseId,FLAGS,XGR,XGSJ,AsText(geometry) as Wkt FROM DXTDLSSZJ WHERE [FLAGS]<3";
        
		private const string SQL_INSERT_DXTDLSSZJ = "INSERT INTO [DXTDLSSZJ] (WBNR, TC, CASSDM, FH, FHDX, XZJD, YSDM, DatabaseId, FLAGS, XGR, XGSJ, geometry) VALUES ( @WBNR, @TC, @CASSDM, @FH, @FHDX, @XZJD, @YSDM, @DatabaseId, @FLAGS, @XGR, @XGSJ, GeomFromText(@geometry,@SRID));" + " SELECT last_insert_rowid();";
		
		private const string SQL_UPDATE_DXTDLSSZJ = "UPDATE [DXTDLSSZJ] SET WBNR = @WBNR, TC = @TC, CASSDM = @CASSDM, FH = @FH, FHDX = @FHDX, XZJD = @XZJD, YSDM = @YSDM, DatabaseId = @DatabaseId, FLAGS = @FLAGS, XGR = @XGR, XGSJ = @XGSJ,  geometry = GeomFromText(@geometry,@SRID) WHERE Id = @Id";
		
		private const string SQL_DELETE_DXTDLSSZJ = "DELETE FROM [DXTDLSSZJ] WHERE  Id = @Id ";
		
        private const string SQL_DELETE_FLAG_DXTDLSSZJ = "UPDATE [DXTDLSSZJ] Set Flags=3,XGR = @XGR, XGSJ = @XGSJ WHERE WHERE  Id = @Id ";
        #endregion
        ...
        public string SimpleLabelString
        {
            get
            {
                return string.Format("{0} {1} ({2})","DXTDLSSZJ",this.id,this.ysdm);
            }
        }
        ...
        public string FullLabelString
        {
            get
            {
                return string.Format("{0} {1} ({2})","道路设施注记",this.id,this.ysdm);
            }
        }        
        #endregion
       ...
       public IGeometry Geometry
        ...
        {
            Dxtdlsszj record = obj as Dxtdlsszj;           
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_INSERT_DXTDLSSZJ,dao.Connection))
            {
                this.databaseid=0;
                this.flags=1;
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_UPDATE_DXTDLSSZJ,dao.Connection))
            {
        ...
        public bool Delete(ISQLiteService dao)
        {
            if(this.databaseid==0)
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_DXTDLSSZJ,dao.Connection))
                {                   
    				command.Parameters.AddWithValue(PARAM_ID, this.ID);
                    return (command.ExecuteNonQuery() == 1);
                }
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_FLAG_DXTDLSSZJ,dao.Connection))
                {
        ...
        public IEntity Copy()
        {
            Dxtdlsszj target=new Dxtdlsszj();
            target.ID=0;
