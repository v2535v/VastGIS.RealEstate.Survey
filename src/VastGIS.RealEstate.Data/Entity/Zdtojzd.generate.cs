...
{
    public partial class Zdtojzd : INotifyPropertyChanging, INotifyPropertyChanged,IDatabaseEntity
    {		
		#region 表结构		
        public const string TABLE_NAME = "ZDTOJZD";	
        public const string LAYER_NAME = "宗地界址点关系表";	
		...
		#region SQL语句
		private const string SQL_QUERY_ZDTOJZD ="SELECT  Id,ZD_WYDM,JZD_WYDM,SXH,DatabaseId,FLAGS,XGR,XGSJ FROM ZDTOJZD WHERE [FLAGS]<3";
        
		private const string SQL_INSERT_ZDTOJZD = "INSERT INTO [ZDTOJZD] (ZD_WYDM, JZD_WYDM, SXH, DatabaseId, FLAGS, XGR, XGSJ) VALUES ( @ZD_WYDM, @JZD_WYDM, @SXH, @DatabaseId, @FLAGS, @XGR, @XGSJ);" + " SELECT last_insert_rowid();";
		
		private const string SQL_UPDATE_ZDTOJZD = "UPDATE [ZDTOJZD] SET ZD_WYDM = @ZD_WYDM, JZD_WYDM = @JZD_WYDM, SXH = @SXH, DatabaseId = @DatabaseId, FLAGS = @FLAGS, XGR = @XGR, XGSJ = @XGSJ WHERE Id = @Id";
		
		private const string SQL_DELETE_ZDTOJZD = "DELETE FROM [ZDTOJZD] WHERE  Id = @Id ";
		
        private const string SQL_DELETE_FLAG_ZDTOJZD = "UPDATE [ZDTOJZD] Set Flags=3,XGR = @XGR, XGSJ = @XGSJ WHERE WHERE  Id = @Id ";
        #endregion
        ...
        ///界址点唯一代码
		protected string jzdWydm = default(string);
        ///顺序号
        ...
        public string FullLabelString
        {
            get
            {
                return string.Format("{0} {1} ","宗地界址点关系表",this.id);
            }
        }        
        #endregion
        ...
        ///界址点唯一代码
        ///[Column(COL_JZD_WYDM, PARAM_JZD_WYDM )]
        public virtual string JzdWydm 
        {
       ...
       ///图形类型
        ...
        public bool Delete(ISQLiteService dao)
        {
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_FLAG_ZDTOJZD,dao.Connection))
                {
        ...
        public void  LoadFromReader(SQLiteDataReader reader)
        {
			...
			if (!reader.IsDBNull(2)) jzdWydm = reader.GetString(2);
        ...
        public IEntity Copy()
        {
            Zdtojzd target=new Zdtojzd();
            target.ID=0;
            target.ZdWydm=this.ZdWydm;
            target.JzdWydm=this.JzdWydm;
            target.Sxh=this.Sxh;
            target.DatabaseID=0;
            target.Flags=1;
