...
{
    public partial class Dxtdmtzx : INotifyPropertyChanging, INotifyPropertyChanged,IBasemapPolyline,IDatabaseFeature,IYsdmEntity
    {		
		#region 表结构		
		...
		#region SQL语句
		private const string SQL_QUERY_DXTDMTZX ="SELECT  Id,TC,CASSDM,FH,FHDX,FSXX1,FSXX2,YSDM,DatabaseId,FLAGS,XGR,XGSJ,AsText(geometry) as Wkt FROM DXTDMTZX WHERE [FLAGS]<3";
        
		private const string SQL_INSERT_DXTDMTZX = "INSERT INTO [DXTDMTZX] (TC, CASSDM, FH, FHDX, FSXX1, FSXX2, YSDM, DatabaseId, FLAGS, XGR, XGSJ, geometry) VALUES ( @TC, @CASSDM, @FH, @FHDX, @FSXX1, @FSXX2, @YSDM, @DatabaseId, @FLAGS, @XGR, @XGSJ, GeomFromText(@geometry,@SRID));" + " SELECT last_insert_rowid();";
		
		private const string SQL_UPDATE_DXTDMTZX = "UPDATE [DXTDMTZX] SET TC = @TC, CASSDM = @CASSDM, FH = @FH, FHDX = @FHDX, FSXX1 = @FSXX1, FSXX2 = @FSXX2, YSDM = @YSDM, DatabaseId = @DatabaseId, FLAGS = @FLAGS, XGR = @XGR, XGSJ = @XGSJ,  geometry = GeomFromText(@geometry,@SRID) WHERE Id = @Id";
		
		private const string SQL_DELETE_DXTDMTZX = "DELETE FROM [DXTDMTZX] WHERE  Id = @Id ";
		
        private const string SQL_DELETE_FLAG_DXTDMTZX = "UPDATE [DXTDMTZX] Set Flags=3,XGR = @XGR, XGSJ = @XGSJ WHERE WHERE  Id = @Id ";
        #endregion
        ...
        public string FullLabelString
        {
            get
            {
                return string.Format("{0} {1} ({2})","地貌土质线",this.id,this.ysdm);
            }
        }        
        #endregion
       ...
       public IGeometry Geometry
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_INSERT_DXTDMTZX,dao.Connection))
            {
                this.databaseid=0;
                this.flags=1;
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_UPDATE_DXTDMTZX,dao.Connection))
            {
        ...
        public bool Delete(ISQLiteService dao)
        {
            if(this.databaseid==0)
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_DXTDMTZX,dao.Connection))
                {                   
    				command.Parameters.AddWithValue(PARAM_ID, this.ID);
                    return (command.ExecuteNonQuery() == 1);
                }
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_FLAG_DXTDMTZX,dao.Connection))
                {
