...
namespace VastGIS.RealEstate.Data.Entity
{
    ...
    public partial class Dxtjmdx:INotifyPropertyChanging, INotifyPropertyChanged,IDatabaseEntity,IBasemapPolyline
    {
        #region 表结构
	    ...
	    private const string SQL_INSERT_DXTJMDX = "INSERT INTO DXTJMDX (TC, CASSDM, FH, FHDX, FSXX1, FSXX2, YSDM, DatabaseId, FLAGS, geometry) VALUES ( @TC, @CASSDM, @FH, @FHDX, @FSXX1, @FSXX2, @YSDM, @DatabaseId, @FLAGS, GeomFromText(@geometry,@SRID));" + " SELECT last_insert_rowid();";
	
	    private const string SQL_UPDATE_DXTJMDX = "UPDATE DXTJMDX SET TC = @TC, CASSDM = @CASSDM, FH = @FH, FHDX = @FHDX, FSXX1 = @FSXX1, FSXX2 = @FSXX2, YSDM = @YSDM, DatabaseId = @DatabaseId, FLAGS = @FLAGS, geometry = GeomFromText(@geometry,@SRID) WHERE Id = @Id";
	
	    private const string SQL_DELETE_DXTJMDX = "DELETE FROM DXTJMDX WHERE  Id = @Id ";
        
        private const string SQL_DELETE_FLAG_DXTJMDX = "UPDATE DXTJMDX Set Flags=3 WHERE  Id = @Id ";
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_INSERT_DXTJMDX,connection))
            {	
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_UPDATE_DXTJMDX,connection))
            {							
		...
		public bool Delete(SQLiteConnection connection)
        {
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_DXTJMDX,connection))
                {
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_FLAG_DXTJMDX,connection))
                {
