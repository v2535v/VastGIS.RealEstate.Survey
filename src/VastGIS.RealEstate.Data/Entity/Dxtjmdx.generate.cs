...
{
    public partial class Dxtjmdx : INotifyPropertyChanging, INotifyPropertyChanged,IBasemapPolyline,IDatabaseFeature,IYsdmEntity
    {		
		#region 表结构		
        public const string TABLE_NAME = "DXTJMDX";	
        public const string LAYER_NAME = "居民地线";	
		...
		#region SQL语句
		private const string SQL_QUERY_DXTJMDX ="SELECT  Id,TC,CASSDM,FH,FHDX,FSXX1,FSXX2,YSDM,DatabaseId,FLAGS,XGR,XGSJ,AsText(geometry) as Wkt FROM DXTJMDX WHERE [FLAGS]<3";
        
		private const string SQL_INSERT_DXTJMDX = "INSERT INTO [DXTJMDX] (TC, CASSDM, FH, FHDX, FSXX1, FSXX2, YSDM, DatabaseId, FLAGS, XGR, XGSJ, geometry) VALUES ( @TC, @CASSDM, @FH, @FHDX, @FSXX1, @FSXX2, @YSDM, @DatabaseId, @FLAGS, @XGR, @XGSJ, GeomFromText(@geometry,@SRID));" + " SELECT last_insert_rowid();";
		
		private const string SQL_UPDATE_DXTJMDX = "UPDATE [DXTJMDX] SET TC = @TC, CASSDM = @CASSDM, FH = @FH, FHDX = @FHDX, FSXX1 = @FSXX1, FSXX2 = @FSXX2, YSDM = @YSDM, DatabaseId = @DatabaseId, FLAGS = @FLAGS, XGR = @XGR, XGSJ = @XGSJ,  geometry = GeomFromText(@geometry,@SRID) WHERE Id = @Id";
		
		private const string SQL_DELETE_DXTJMDX = "DELETE FROM [DXTJMDX] WHERE  Id = @Id ";
		
        private const string SQL_DELETE_FLAG_DXTJMDX = "UPDATE [DXTJMDX] Set Flags=3,XGR = @XGR, XGSJ = @XGSJ WHERE WHERE  Id = @Id ";
        #endregion
        ...
        public string SimpleLabelString
        {
            get
            {
                return string.Format("{0} {1} ({2})","DXTJMDX",this.id,this.ysdm);
            }
        }
        ...
        public string FullLabelString
        {
            get
            {
                return string.Format("{0} {1} ({2})","居民地线",this.id,this.ysdm);
            }
        }        
        #endregion
       ...
       public IGeometry Geometry
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_INSERT_DXTJMDX,dao.Connection))
            {
                this.databaseid=0;
                this.flags=1;
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_UPDATE_DXTJMDX,dao.Connection))
            {
        ...
        public bool Delete(ISQLiteService dao)
        {
            if(this.databaseid==0)
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_DXTJMDX,dao.Connection))
                {                   
    				command.Parameters.AddWithValue(PARAM_ID, this.ID);
                    return (command.ExecuteNonQuery() == 1);
                }
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_FLAG_DXTJMDX,dao.Connection))
                {
