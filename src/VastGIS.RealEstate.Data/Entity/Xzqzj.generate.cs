...
{
    public partial class Xzqzj : INotifyPropertyChanging, INotifyPropertyChanged,IDatabaseFeature,IYsdmEntity
    {		
		#region 表结构		
        public const string TABLE_NAME = "XZQZJ";	
        public const string LAYER_NAME = "行政区注记";	
		...
		#region SQL语句
		private const string SQL_QUERY_XZQZJ ="SELECT  Id,YSDM,ZJNR,ZT,YS,BS,XZ,XHX,KD,GD,ZJDZXJXZB,ZJDZXJYZB,ZJFX,DatabaseId,FLAGS,XGR,XGSJ,AsText(geometry) as Wkt FROM XZQZJ WHERE [FLAGS]<3";
        
		private const string SQL_INSERT_XZQZJ = "INSERT INTO [XZQZJ] (YSDM, ZJNR, ZT, YS, BS, XZ, XHX, KD, GD, ZJDZXJXZB, ZJDZXJYZB, ZJFX, DatabaseId, FLAGS, XGR, XGSJ, geometry) VALUES ( @YSDM, @ZJNR, @ZT, @YS, @BS, @XZ, @XHX, @KD, @GD, @ZJDZXJXZB, @ZJDZXJYZB, @ZJFX, @DatabaseId, @FLAGS, @XGR, @XGSJ, GeomFromText(@geometry,@SRID));" + " SELECT last_insert_rowid();";
		
		private const string SQL_UPDATE_XZQZJ = "UPDATE [XZQZJ] SET YSDM = @YSDM, ZJNR = @ZJNR, ZT = @ZT, YS = @YS, BS = @BS, XZ = @XZ, XHX = @XHX, KD = @KD, GD = @GD, ZJDZXJXZB = @ZJDZXJXZB, ZJDZXJYZB = @ZJDZXJYZB, ZJFX = @ZJFX, DatabaseId = @DatabaseId, FLAGS = @FLAGS, XGR = @XGR, XGSJ = @XGSJ,  geometry = GeomFromText(@geometry,@SRID) WHERE Id = @Id";
		
		private const string SQL_DELETE_XZQZJ = "DELETE FROM [XZQZJ] WHERE  Id = @Id ";
		
        private const string SQL_DELETE_FLAG_XZQZJ = "UPDATE [XZQZJ] Set Flags=3,XGR = @XGR, XGSJ = @XGSJ WHERE WHERE  Id = @Id ";
        #endregion
        ...
        public string SimpleLabelString
        {
            get
            {
                return string.Format("{0} {1} ({2})","XZQZJ",this.id,this.ysdm);
            }
        }
        ...
        public string FullLabelString
        {
            get
            {
                return string.Format("{0} {1} ({2})","行政区注记",this.id,this.ysdm);
            }
        }        
        #endregion
       ...
       public IGeometry Geometry
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_INSERT_XZQZJ,dao.Connection))
            {
                this.databaseid=0;
                this.flags=1;
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_UPDATE_XZQZJ,dao.Connection))
            {
        ...
        public bool Delete(ISQLiteService dao)
        {
            if(this.databaseid==0)
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_XZQZJ,dao.Connection))
                {                   
    				command.Parameters.AddWithValue(PARAM_ID, this.ID);
                    return (command.ExecuteNonQuery() == 1);
                }
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_FLAG_XZQZJ,dao.Connection))
                {
        ...
        public void  LoadFromReader(SQLiteDataReader reader)
        {
			...
			if (!reader.IsDBNull(2)) zjnr = reader.GetString(2);
			if (!reader.IsDBNull(3)) zt = reader.GetString(3);
			if (!reader.IsDBNull(4)) ys = reader.GetString(4);
			if (!reader.IsDBNull(5)) bs = reader.GetInt64(5);
			if (!reader.IsDBNull(6)) xz = reader.GetString(6);
			if (!reader.IsDBNull(7)) xhx = reader.GetString(7);
			if (!reader.IsDBNull(8)) kd = reader.GetDouble(8);
			if (!reader.IsDBNull(9)) gd = reader.GetDouble(9);
			if (!reader.IsDBNull(10)) zjdzxjxzb = reader.GetDouble(10);
			if (!reader.IsDBNull(11)) zjdzxjyzb = reader.GetDouble(11);
			if (!reader.IsDBNull(12)) zjfx = reader.GetDouble(12);
