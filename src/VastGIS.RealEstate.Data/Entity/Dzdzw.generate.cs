...
{
    public partial class Dzdzw : INotifyPropertyChanging, INotifyPropertyChanged,ISurveyFeature,IYsdmEntity,IGlobalEntity
    {		
		#region 表结构		
        public const string TABLE_NAME = "DZDZW";	
        public const string LAYER_NAME = "点状定着物";	
		...
		public const string COL_DZDZWLX = "DZDZWLX";
        ...
        public const string PARAM_DZDZWLX = "@DZDZWLX";	
		...
		#region SQL语句
		private const string SQL_QUERY_DZDZW ="SELECT  Id,YSDM,BDCDYH,ZDZHDM,DZDZWLX,DZWMC,MJDW,MJ,DAH,ZT,WX_DCY,WX_DCSJ,WX_CLY,WX_CLSJ,WX_ZTY,WX_ZTSJ,WX_ZJY,WX_ZJSJ,WX_WYDM,DatabaseId,FLAGS,XGR,XGSJ,AsText(geometry) as Wkt FROM DZDZW WHERE [FLAGS]<3";
        
		private const string SQL_INSERT_DZDZW = "INSERT INTO [DZDZW] (YSDM, BDCDYH, ZDZHDM, DZDZWLX, DZWMC, MJDW, MJ, DAH, ZT, WX_DCY, WX_DCSJ, WX_CLY, WX_CLSJ, WX_ZTY, WX_ZTSJ, WX_ZJY, WX_ZJSJ, WX_WYDM, DatabaseId, FLAGS, XGR, XGSJ, geometry) VALUES ( @YSDM, @BDCDYH, @ZDZHDM, @DZDZWLX, @DZWMC, @MJDW, @MJ, @DAH, @ZT, @WX_DCY, @WX_DCSJ, @WX_CLY, @WX_CLSJ, @WX_ZTY, @WX_ZTSJ, @WX_ZJY, @WX_ZJSJ, @WX_WYDM, @DatabaseId, @FLAGS, @XGR, @XGSJ, GeomFromText(@geometry,@SRID));" + " SELECT last_insert_rowid();";
		
		private const string SQL_UPDATE_DZDZW = "UPDATE [DZDZW] SET YSDM = @YSDM, BDCDYH = @BDCDYH, ZDZHDM = @ZDZHDM, DZDZWLX = @DZDZWLX, DZWMC = @DZWMC, MJDW = @MJDW, MJ = @MJ, DAH = @DAH, ZT = @ZT, WX_DCY = @WX_DCY, WX_DCSJ = @WX_DCSJ, WX_CLY = @WX_CLY, WX_CLSJ = @WX_CLSJ, WX_ZTY = @WX_ZTY, WX_ZTSJ = @WX_ZTSJ, WX_ZJY = @WX_ZJY, WX_ZJSJ = @WX_ZJSJ, WX_WYDM = @WX_WYDM, DatabaseId = @DatabaseId, FLAGS = @FLAGS, XGR = @XGR, XGSJ = @XGSJ,  geometry = GeomFromText(@geometry,@SRID) WHERE Id = @Id";
		
		private const string SQL_DELETE_DZDZW = "DELETE FROM [DZDZW] WHERE  Id = @Id ";
		
        private const string SQL_DELETE_FLAG_DZDZW = "UPDATE [DZDZW] Set Flags=3,XGR = @XGR, XGSJ = @XGSJ WHERE WHERE  Id = @Id ";
        #endregion
        ...
        ///点状定着物类型
		protected string dzdzwlx = default(string);
        ///定着物名称
        ...
        public string SimpleLabelString
        {
            get
            {
                return string.Format("{0} {1} ({2})","DZDZW",this.id,this.ysdm);
            }
        }
        ...
        public string FullLabelString
        {
            get
            {
                return string.Format("{0} {1} ({2})","点状定着物",this.id,this.ysdm);
            }
        }        
        #endregion
        ...
        ///点状定着物类型
        ///[Column(COL_DZDZWLX, PARAM_DZDZWLX )]
        public virtual string Dzdzwlx 
        {
            get { return this.dzdzwlx; }
			set	{ 
                  if(this.dzdzwlx != value)
                    {
                        this.OnPropertyChanging("Dzdzwlx"); 
                        this.dzdzwlx = value;                        
                        this.OnPropertyChanged("Dzdzwlx");
                    }   
                }
        }	
       ...
       public IGeometry Geometry
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_INSERT_DZDZW,dao.Connection))
            {
                this.databaseid=0;
                this.flags=1;
				...
				command.Parameters.AddWithValue(PARAM_DZDZWLX, this.Dzdzwlx);
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_UPDATE_DZDZW,dao.Connection))
            {
        ...
        public bool Delete(ISQLiteService dao)
        {
            if(this.databaseid==0)
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_DZDZW,dao.Connection))
                {                   
    				command.Parameters.AddWithValue(PARAM_ID, this.ID);
                    return (command.ExecuteNonQuery() == 1);
                }
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_FLAG_DZDZW,dao.Connection))
                {
        ...
        public void  LoadFromReader(SQLiteDataReader reader)
        {
			...
			if (!reader.IsDBNull(4)) dzdzwlx = reader.GetString(4);
