...
{
    public partial class TmpCadd : INotifyPropertyChanging, INotifyPropertyChanged,IDatabaseFeature
    {		
		#region 表结构		
        public const string TABLE_NAME = "TmpCadd";	
        public const string LAYER_NAME = "TmpCadd";	
		...
		#region SQL语句
		private const string SQL_QUERY_TMPCADD ="SELECT  Id,EntityType,Handle,FileName,DatabaseId,FLAGS,XGR,XGSJ,AsText(geometry) as Wkt FROM TmpCadd WHERE [FLAGS]<3";
        
		private const string SQL_INSERT_TMPCADD = "INSERT INTO [TmpCadd] (EntityType, Handle, FileName, DatabaseId, FLAGS, XGR, XGSJ, geometry) VALUES ( @EntityType, @Handle, @FileName, @DatabaseId, @FLAGS, @XGR, @XGSJ, GeomFromText(@geometry,@SRID));" + " SELECT last_insert_rowid();";
		
		private const string SQL_UPDATE_TMPCADD = "UPDATE [TmpCadd] SET EntityType = @EntityType, Handle = @Handle, FileName = @FileName, DatabaseId = @DatabaseId, FLAGS = @FLAGS, XGR = @XGR, XGSJ = @XGSJ,  geometry = GeomFromText(@geometry,@SRID) WHERE Id = @Id";
		
		private const string SQL_DELETE_TMPCADD = "DELETE FROM [TmpCadd] WHERE  Id = @Id ";
		
        private const string SQL_DELETE_FLAG_TMPCADD = "UPDATE [TmpCadd] Set Flags=3,XGR = @XGR, XGSJ = @XGSJ WHERE WHERE  Id = @Id ";
        #endregion
        ...
        public string SimpleLabelString
        {
            get
            {
                return string.Format("{0} {1} ","TmpCadd",this.id);
            }
        }
       ...
       public IGeometry Geometry
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_INSERT_TMPCADD,dao.Connection))
            {
                this.databaseid=0;
                this.flags=1;
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_UPDATE_TMPCADD,dao.Connection))
            {
        ...
        public bool Delete(ISQLiteService dao)
        {
            if(this.databaseid==0)
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_TMPCADD,dao.Connection))
                {                   
    				command.Parameters.AddWithValue(PARAM_ID, this.ID);
                    return (command.ExecuteNonQuery() == 1);
                }
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_FLAG_TMPCADD,dao.Connection))
                {
