...
namespace VastGIS.RealEstate.Data.Entity
{
    ...
    public partial class Dxtkzdd:INotifyPropertyChanging, INotifyPropertyChanged,IDatabaseEntity,IBasemapPoint
    {
        #region 表结构
	    ...
	    private const string SQL_INSERT_DXTKZDD = "INSERT INTO DXTKZDD (TC, CASSDM, FH, FHDX, XZJD, FSXX1, FSXX2, YSDM, DatabaseId, FLAGS, geometry) VALUES ( @TC, @CASSDM, @FH, @FHDX, @XZJD, @FSXX1, @FSXX2, @YSDM, @DatabaseId, @FLAGS, GeomFromText(@geometry,@SRID));" + " SELECT last_insert_rowid();";
	
	    private const string SQL_UPDATE_DXTKZDD = "UPDATE DXTKZDD SET TC = @TC, CASSDM = @CASSDM, FH = @FH, FHDX = @FHDX, XZJD = @XZJD, FSXX1 = @FSXX1, FSXX2 = @FSXX2, YSDM = @YSDM, DatabaseId = @DatabaseId, FLAGS = @FLAGS, geometry = GeomFromText(@geometry,@SRID) WHERE Id = @Id";
	
	    private const string SQL_DELETE_DXTKZDD = "DELETE FROM DXTKZDD WHERE  Id = @Id ";
        
        private const string SQL_DELETE_FLAG_DXTKZDD = "UPDATE DXTKZDD Set Flags=3 WHERE  Id = @Id ";
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_INSERT_DXTKZDD,connection))
            {	
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_UPDATE_DXTKZDD,connection))
            {							
		...
		public bool Delete(SQLiteConnection connection)
        {
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_DXTKZDD,connection))
                {
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_FLAG_DXTKZDD,connection))
                {
