...
{
    public partial class Dxtzjzj : INotifyPropertyChanging, INotifyPropertyChanged,IBasemapText,IDatabaseFeature,IYsdmEntity
    {		
		#region 表结构		
		...
		#region SQL语句
		private const string SQL_QUERY_DXTZJZJ ="SELECT  Id,WBNR,TC,CASSDM,FH,FHDX,XZJD,YSDM,DatabaseId,FLAGS,XGR,XGSJ,AsText(geometry) as Wkt FROM DXTZJZJ WHERE [FLAGS]<3";
        ...
        private const string SQL_DELETE_FLAG_DXTZJZJ = "UPDATE [DXTZJZJ] Set Flags=3,XGR = @XGR, XGSJ = @XGSJ WHERE WHERE  Id = @Id ";
        #endregion
        ...
        public string FullLabelString
        {
            get
            {
                return string.Format("{0} {1} ({2})","注记注记",this.id,this.ysdm);
            }
        }        
        #endregion
