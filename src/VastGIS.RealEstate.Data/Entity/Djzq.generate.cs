...
{
    public partial class Djzq : INotifyPropertyChanging, INotifyPropertyChanged,ISurveyFeature,IYsdmEntity,IGlobalEntity
    {		
		#region 表结构		
        public const string TABLE_NAME = "DJZQ";	
        public const string LAYER_NAME = "地籍子区";	
		...
		#region SQL语句
		private const string SQL_QUERY_DJZQ ="SELECT  Id,YSDM,DJZQDM,DJZQMC,WX_DCY,WX_DCSJ,WX_CLY,WX_CLSJ,WX_ZTY,WX_ZTSJ,WX_ZJY,WX_ZJSJ,WX_WYDM,DatabaseId,FLAGS,XGR,XGSJ,AsText(geometry) as Wkt FROM DJZQ WHERE [FLAGS]<3";
        
		private const string SQL_INSERT_DJZQ = "INSERT INTO [DJZQ] (YSDM, DJZQDM, DJZQMC, WX_DCY, WX_DCSJ, WX_CLY, WX_CLSJ, WX_ZTY, WX_ZTSJ, WX_ZJY, WX_ZJSJ, WX_WYDM, DatabaseId, FLAGS, XGR, XGSJ, geometry) VALUES ( @YSDM, @DJZQDM, @DJZQMC, @WX_DCY, @WX_DCSJ, @WX_CLY, @WX_CLSJ, @WX_ZTY, @WX_ZTSJ, @WX_ZJY, @WX_ZJSJ, @WX_WYDM, @DatabaseId, @FLAGS, @XGR, @XGSJ, GeomFromText(@geometry,@SRID));" + " SELECT last_insert_rowid();";
        ...
        private const string SQL_DELETE_FLAG_DJZQ = "UPDATE [DJZQ] Set Flags=3,XGR = @XGR, XGSJ = @XGSJ WHERE WHERE  Id = @Id ";
        #endregion
        ...
        public string SimpleLabelString
        {
            get
            {
                return string.Format("{0} {1} ({2})","DJZQ",this.id,this.ysdm);
            }
        }
        ...
        public string FullLabelString
        {
            get
            {
                return string.Format("{0} {1} ({2})","地籍子区",this.id,this.ysdm);
            }
        }        
        #endregion
        ...
        ///地籍子区代码
        ///[Column(COL_DJZQDM, PARAM_DJZQDM )]
        public virtual string Djzqdm 
        {
        ...
        ///地籍子区名称
        ///[Column(COL_DJZQMC, PARAM_DJZQMC )]
        public virtual string Djzqmc 
        {
       ...
       public IGeometry Geometry
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_INSERT_DJZQ,dao.Connection))
            {
                this.databaseid=0;
                this.flags=1;
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_UPDATE_DJZQ,dao.Connection))
            {
        ...
        public bool Delete(ISQLiteService dao)
        {
            if(this.databaseid==0)
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_DJZQ,dao.Connection))
                {                   
    				command.Parameters.AddWithValue(PARAM_ID, this.ID);
                    return (command.ExecuteNonQuery() == 1);
                }
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_FLAG_DJZQ,dao.Connection))
                {
        ...
        public void  LoadFromReader(SQLiteDataReader reader)
        {
			...
			if (!reader.IsDBNull(2)) djzqdm = reader.GetString(2);
			if (!reader.IsDBNull(3)) djzqmc = reader.GetString(3);
        ...
        public IEntity Copy()
        {
            Djzq target=new Djzq();
            target.ID=0;
            target.Ysdm=this.Ysdm;
            target.Djzqdm=this.Djzqdm;
            target.Djzqmc=this.Djzqmc;
