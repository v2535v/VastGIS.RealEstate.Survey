...
{
    public partial class TmpCadzj : INotifyPropertyChanging, INotifyPropertyChanged,IDatabaseFeature
    {		
		#region 表结构		
        public const string TABLE_NAME = "TmpCadzj";	
        public const string LAYER_NAME = "TmpCadzj";	
		...
		#region SQL语句
		private const string SQL_QUERY_TMPCADZJ ="SELECT  Id,EntityType,Handle,FileName,DatabaseId,FLAGS,XGR,XGSJ,AsText(geometry) as Wkt FROM TmpCadzj WHERE [FLAGS]<3";
        
		private const string SQL_INSERT_TMPCADZJ = "INSERT INTO [TmpCadzj] (EntityType, Handle, FileName, DatabaseId, FLAGS, XGR, XGSJ, geometry) VALUES ( @EntityType, @Handle, @FileName, @DatabaseId, @FLAGS, @XGR, @XGSJ, GeomFromText(@geometry,@SRID));" + " SELECT last_insert_rowid();";
		
		private const string SQL_UPDATE_TMPCADZJ = "UPDATE [TmpCadzj] SET EntityType = @EntityType, Handle = @Handle, FileName = @FileName, DatabaseId = @DatabaseId, FLAGS = @FLAGS, XGR = @XGR, XGSJ = @XGSJ,  geometry = GeomFromText(@geometry,@SRID) WHERE Id = @Id";
		
		private const string SQL_DELETE_TMPCADZJ = "DELETE FROM [TmpCadzj] WHERE  Id = @Id ";
		
        private const string SQL_DELETE_FLAG_TMPCADZJ = "UPDATE [TmpCadzj] Set Flags=3,XGR = @XGR, XGSJ = @XGSJ WHERE WHERE  Id = @Id ";
        #endregion
       ...
       public IGeometry Geometry
        ...
        {
            TmpCadzj record = obj as TmpCadzj;           
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_INSERT_TMPCADZJ,dao.Connection))
            {
                this.databaseid=0;
                this.flags=1;
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_UPDATE_TMPCADZJ,dao.Connection))
            {
        ...
        public bool Delete(ISQLiteService dao)
        {
            if(this.databaseid==0)
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_TMPCADZJ,dao.Connection))
                {                   
    				command.Parameters.AddWithValue(PARAM_ID, this.ID);
                    return (command.ExecuteNonQuery() == 1);
                }
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_FLAG_TMPCADZJ,dao.Connection))
                {
