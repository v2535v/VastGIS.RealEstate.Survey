...
namespace VastGIS.RealEstate.Data.Entity
{
    ...
    public partial class Dxtzjm:INotifyPropertyChanging, INotifyPropertyChanged,IDatabaseEntity,IBasemapPolygon
    {
        #region 表结构
	    ...
	    private const string SQL_INSERT_DXTZJM = "INSERT INTO DXTZJM (TC, CASSDM, FSXX1, FSXX2, YSDM, DatabaseId, FLAGS, geometry) VALUES ( @TC, @CASSDM, @FSXX1, @FSXX2, @YSDM, @DatabaseId, @FLAGS, GeomFromText(@geometry,@SRID));" + " SELECT last_insert_rowid();";
	
	    private const string SQL_UPDATE_DXTZJM = "UPDATE DXTZJM SET TC = @TC, CASSDM = @CASSDM, FSXX1 = @FSXX1, FSXX2 = @FSXX2, YSDM = @YSDM, DatabaseId = @DatabaseId, FLAGS = @FLAGS, geometry = GeomFromText(@geometry,@SRID) WHERE Id = @Id";
	
	    private const string SQL_DELETE_DXTZJM = "DELETE FROM DXTZJM WHERE  Id = @Id ";
        
        private const string SQL_DELETE_FLAG_DXTZJM = "UPDATE DXTZJM Set Flags=3 WHERE  Id = @Id ";
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_INSERT_DXTZJM,connection))
            {	
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_UPDATE_DXTZJM,connection))
            {							
		...
		public bool Delete(SQLiteConnection connection)
        {
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_DXTZJM,connection))
                {
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_FLAG_DXTZJM,connection))
                {
