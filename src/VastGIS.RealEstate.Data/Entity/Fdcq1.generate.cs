...
{
    public partial class Fdcq1 : INotifyPropertyChanging, INotifyPropertyChanged,IDatabaseEntity,IYsdmEntity,IGlobalEntity,ISurveyEntity
    {		
		#region 表结构		
        public const string TABLE_NAME = "FDCQ1";	
        public const string LAYER_NAME = "房地产权（项目内多幢房屋）";	
		...
		#region SQL语句
		private const string SQL_QUERY_FDCQ1 ="SELECT  Id,YSDM,BDCDYH,YWH,QLLX,DJLX,DJYY,FDZL,TDSYQR,DYTDMJ,FTTDMJ,TDSYQSSJ,TDSYJSSJ,FDCJYJG,BDCQZH,QXDM,DJJG,DBR,DJSJ,FJ,FCFHT,QSZT,WX_DCY,WX_DCSJ,WX_CLY,WX_CLSJ,WX_ZTY,WX_ZTSJ,WX_ZJY,WX_ZJSJ,WX_WYDM,DatabaseId,FLAGS,XGR,XGSJ FROM FDCQ1 WHERE [FLAGS]<3";
        
		private const string SQL_INSERT_FDCQ1 = "INSERT INTO [FDCQ1] (YSDM, BDCDYH, YWH, QLLX, DJLX, DJYY, FDZL, TDSYQR, DYTDMJ, FTTDMJ, TDSYQSSJ, TDSYJSSJ, FDCJYJG, BDCQZH, QXDM, DJJG, DBR, DJSJ, FJ, FCFHT, QSZT, WX_DCY, WX_DCSJ, WX_CLY, WX_CLSJ, WX_ZTY, WX_ZTSJ, WX_ZJY, WX_ZJSJ, WX_WYDM, DatabaseId, FLAGS, XGR, XGSJ) VALUES ( @YSDM, @BDCDYH, @YWH, @QLLX, @DJLX, @DJYY, @FDZL, @TDSYQR, @DYTDMJ, @FTTDMJ, @TDSYQSSJ, @TDSYJSSJ, @FDCJYJG, @BDCQZH, @QXDM, @DJJG, @DBR, @DJSJ, @FJ, @FCFHT, @QSZT, @WX_DCY, @WX_DCSJ, @WX_CLY, @WX_CLSJ, @WX_ZTY, @WX_ZTSJ, @WX_ZJY, @WX_ZJSJ, @WX_WYDM, @DatabaseId, @FLAGS, @XGR, @XGSJ);" + " SELECT last_insert_rowid();";
		
		private const string SQL_UPDATE_FDCQ1 = "UPDATE [FDCQ1] SET YSDM = @YSDM, BDCDYH = @BDCDYH, YWH = @YWH, QLLX = @QLLX, DJLX = @DJLX, DJYY = @DJYY, FDZL = @FDZL, TDSYQR = @TDSYQR, DYTDMJ = @DYTDMJ, FTTDMJ = @FTTDMJ, TDSYQSSJ = @TDSYQSSJ, TDSYJSSJ = @TDSYJSSJ, FDCJYJG = @FDCJYJG, BDCQZH = @BDCQZH, QXDM = @QXDM, DJJG = @DJJG, DBR = @DBR, DJSJ = @DJSJ, FJ = @FJ, FCFHT = @FCFHT, QSZT = @QSZT, WX_DCY = @WX_DCY, WX_DCSJ = @WX_DCSJ, WX_CLY = @WX_CLY, WX_CLSJ = @WX_CLSJ, WX_ZTY = @WX_ZTY, WX_ZTSJ = @WX_ZTSJ, WX_ZJY = @WX_ZJY, WX_ZJSJ = @WX_ZJSJ, WX_WYDM = @WX_WYDM, DatabaseId = @DatabaseId, FLAGS = @FLAGS, XGR = @XGR, XGSJ = @XGSJ WHERE Id = @Id";
		
		private const string SQL_DELETE_FDCQ1 = "DELETE FROM [FDCQ1] WHERE  Id = @Id ";
		
        private const string SQL_DELETE_FLAG_FDCQ1 = "UPDATE [FDCQ1] Set Flags=3,XGR = @XGR, XGSJ = @XGSJ WHERE WHERE  Id = @Id ";
        #endregion
        ...
        public string SimpleLabelString
        {
            get
            {
                return string.Format("{0} {1} ({2})","FDCQ1",this.id,this.ysdm);
            }
        }
        ...
        public string FullLabelString
        {
            get
            {
                return string.Format("{0} {1} ({2})","房地产权（项目内多幢房屋）",this.id,this.ysdm);
            }
        }        
        #endregion
       ...
       ///图形类型
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_INSERT_FDCQ1,dao.Connection))
            {
                this.databaseid=0;
                this.flags=1;
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_UPDATE_FDCQ1,dao.Connection))
            {
        ...
        public bool Delete(ISQLiteService dao)
        {
            if(this.databaseid==0)
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_FDCQ1,dao.Connection))
                {                   
    				command.Parameters.AddWithValue(PARAM_ID, this.ID);
                    return (command.ExecuteNonQuery() == 1);
                }
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_FLAG_FDCQ1,dao.Connection))
                {
        ...
        public void  LoadFromReader(SQLiteDataReader reader)
        {
			...
			if (!reader.IsDBNull(20)) fcfht = SQLiteHelper.GetBytes(reader,"FCFHT");
			if (!reader.IsDBNull(21)) qszt = reader.GetString(21);
			if (!reader.IsDBNull(22)) wxDcy = reader.GetString(22);
			if (!reader.IsDBNull(23)) wxDcsj = reader.GetDateTime(23);
			if (!reader.IsDBNull(24)) wxCly = reader.GetString(24);
			if (!reader.IsDBNull(25)) wxClsj = reader.GetDateTime(25);
			if (!reader.IsDBNull(26)) wxZty = reader.GetString(26);
			if (!reader.IsDBNull(27)) wxZtsj = reader.GetDateTime(27);
			if (!reader.IsDBNull(28)) wxZjy = reader.GetString(28);
			if (!reader.IsDBNull(29)) wxZjsj = reader.GetDateTime(29);
			if (!reader.IsDBNull(30)) wxWydm = reader.GetString(30);
			if (!reader.IsDBNull(31)) databaseid = reader.GetInt64(31);
			if (!reader.IsDBNull(32)) flags = reader.GetInt16(32);
			if (!reader.IsDBNull(33)) xgr = reader.GetString(33);
			if (!reader.IsDBNull(34)) xgsj = reader.GetDateTime(34);
        }
        
        #region 拷贝
        public IEntity Copy()
        {
            Fdcq1 target=new Fdcq1();
            target.ID=0;
