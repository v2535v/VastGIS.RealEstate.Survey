...
namespace VastGIS.RealEstate.Data.Entity
{
    ...
    public partial class Dxtqtzj:INotifyPropertyChanging, INotifyPropertyChanged,IDatabaseEntity,IBasemapText
    {
        #region 表结构
	    ...
	    private const string SQL_INSERT_DXTQTZJ = "INSERT INTO DXTQTZJ (WBNR, TC, CASSDM, FH, FHDX, XZJD, YSDM, DatabaseId, FLAGS, geometry) VALUES ( @WBNR, @TC, @CASSDM, @FH, @FHDX, @XZJD, @YSDM, @DatabaseId, @FLAGS, GeomFromText(@geometry,@SRID));" + " SELECT last_insert_rowid();";
	
	    private const string SQL_UPDATE_DXTQTZJ = "UPDATE DXTQTZJ SET WBNR = @WBNR, TC = @TC, CASSDM = @CASSDM, FH = @FH, FHDX = @FHDX, XZJD = @XZJD, YSDM = @YSDM, DatabaseId = @DatabaseId, FLAGS = @FLAGS, geometry = GeomFromText(@geometry,@SRID) WHERE Id = @Id";
	
	    private const string SQL_DELETE_DXTQTZJ = "DELETE FROM DXTQTZJ WHERE  Id = @Id ";
        
        private const string SQL_DELETE_FLAG_DXTQTZJ = "UPDATE DXTQTZJ Set Flags=3 WHERE  Id = @Id ";
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_INSERT_DXTQTZJ,connection))
            {	
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_UPDATE_DXTQTZJ,connection))
            {							
		...
		public bool Delete(SQLiteConnection connection)
        {
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_DXTQTZJ,connection))
                {
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_FLAG_DXTQTZJ,connection))
                {
