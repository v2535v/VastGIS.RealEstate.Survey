...
{
    public partial class Dxtqtx : INotifyPropertyChanging, INotifyPropertyChanged,IBasemapPolyline,IDatabaseFeature,IYsdmEntity
    {		
		#region 表结构		
		...
		#region SQL语句
		private const string SQL_QUERY_DXTQTX ="SELECT  Id,TC,CASSDM,FH,FHDX,FSXX1,FSXX2,YSDM,DatabaseId,FLAGS,XGR,XGSJ,AsText(geometry) as Wkt FROM DXTQTX WHERE [FLAGS]<3";
        ...
        private const string SQL_DELETE_FLAG_DXTQTX = "UPDATE [DXTQTX] Set Flags=3,XGR = @XGR, XGSJ = @XGSJ WHERE WHERE  Id = @Id ";
        #endregion
        ...
        public string FullLabelString
        {
            get
            {
                return string.Format("{0} {1} ({2})","其他图层线",this.id,this.ysdm);
            }
        }        
        #endregion
