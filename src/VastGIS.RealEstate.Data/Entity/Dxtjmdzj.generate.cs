...
namespace VastGIS.RealEstate.Data.Entity
{
    ...
    public partial class Dxtjmdzj:INotifyPropertyChanging, INotifyPropertyChanged,IDatabaseEntity,IBasemapText
    {
        #region 表结构
	    ...
	    private const string SQL_INSERT_DXTJMDZJ = "INSERT INTO DXTJMDZJ (WBNR, TC, CASSDM, FH, FHDX, XZJD, YSDM, DatabaseId, FLAGS, geometry) VALUES ( @WBNR, @TC, @CASSDM, @FH, @FHDX, @XZJD, @YSDM, @DatabaseId, @FLAGS, GeomFromText(@geometry,@SRID));" + " SELECT last_insert_rowid();";
	
	    private const string SQL_UPDATE_DXTJMDZJ = "UPDATE DXTJMDZJ SET WBNR = @WBNR, TC = @TC, CASSDM = @CASSDM, FH = @FH, FHDX = @FHDX, XZJD = @XZJD, YSDM = @YSDM, DatabaseId = @DatabaseId, FLAGS = @FLAGS, geometry = GeomFromText(@geometry,@SRID) WHERE Id = @Id";
	
	    private const string SQL_DELETE_DXTJMDZJ = "DELETE FROM DXTJMDZJ WHERE  Id = @Id ";
        
        private const string SQL_DELETE_FLAG_DXTJMDZJ = "UPDATE DXTJMDZJ Set Flags=3 WHERE  Id = @Id ";
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_INSERT_DXTJMDZJ,connection))
            {	
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_UPDATE_DXTJMDZJ,connection))
            {							
		...
		public bool Delete(SQLiteConnection connection)
        {
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_DXTJMDZJ,connection))
                {
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_FLAG_DXTJMDZJ,connection))
                {
