...
{
    public partial class Zdjbxxzj : INotifyPropertyChanging, INotifyPropertyChanged,IDatabaseFeature,IYsdmEntity
    {		
		#region 表结构		
        public const string TABLE_NAME = "ZDJBXXZJ";	
        public const string LAYER_NAME = "宗地注记";	
		...
		#region SQL语句
		private const string SQL_QUERY_ZDJBXXZJ ="SELECT  Id,GLYSDM,YSDM,ZJNR,ZT,YS,BS,XZ,XHX,KD,GD,ZJDZXJXZB,ZJDZXJYZB,ZJFX,DatabaseId,FLAGS,XGR,XGSJ,AsText(geometry) as Wkt FROM ZDJBXXZJ WHERE [FLAGS]<3";
        
		private const string SQL_INSERT_ZDJBXXZJ = "INSERT INTO [ZDJBXXZJ] (GLYSDM, YSDM, ZJNR, ZT, YS, BS, XZ, XHX, KD, GD, ZJDZXJXZB, ZJDZXJYZB, ZJFX, DatabaseId, FLAGS, XGR, XGSJ, geometry) VALUES ( @GLYSDM, @YSDM, @ZJNR, @ZT, @YS, @BS, @XZ, @XHX, @KD, @GD, @ZJDZXJXZB, @ZJDZXJYZB, @ZJFX, @DatabaseId, @FLAGS, @XGR, @XGSJ, GeomFromText(@geometry,@SRID));" + " SELECT last_insert_rowid();";
		
		private const string SQL_UPDATE_ZDJBXXZJ = "UPDATE [ZDJBXXZJ] SET GLYSDM = @GLYSDM, YSDM = @YSDM, ZJNR = @ZJNR, ZT = @ZT, YS = @YS, BS = @BS, XZ = @XZ, XHX = @XHX, KD = @KD, GD = @GD, ZJDZXJXZB = @ZJDZXJXZB, ZJDZXJYZB = @ZJDZXJYZB, ZJFX = @ZJFX, DatabaseId = @DatabaseId, FLAGS = @FLAGS, XGR = @XGR, XGSJ = @XGSJ,  geometry = GeomFromText(@geometry,@SRID) WHERE Id = @Id";
		
		private const string SQL_DELETE_ZDJBXXZJ = "DELETE FROM [ZDJBXXZJ] WHERE  Id = @Id ";
		
        private const string SQL_DELETE_FLAG_ZDJBXXZJ = "UPDATE [ZDJBXXZJ] Set Flags=3,XGR = @XGR, XGSJ = @XGSJ WHERE WHERE  Id = @Id ";
        #endregion
        ...
        public string SimpleLabelString
        {
            get
            {
                return string.Format("{0} {1} ({2})","ZDJBXXZJ",this.id,this.ysdm);
            }
        }
        ...
        public string FullLabelString
        {
            get
            {
                return string.Format("{0} {1} ({2})","宗地注记",this.id,this.ysdm);
            }
        }        
        #endregion
        ...
        ///组内要素代码
        ///[Column(COL_GLYSDM, PARAM_GLYSDM )]
        public virtual string Glysdm 
        {
       ...
       public IGeometry Geometry
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_INSERT_ZDJBXXZJ,dao.Connection))
            {
                this.databaseid=0;
                this.flags=1;
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_UPDATE_ZDJBXXZJ,dao.Connection))
            {
        ...
        public bool Delete(ISQLiteService dao)
        {
            if(this.databaseid==0)
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_ZDJBXXZJ,dao.Connection))
                {                   
    				command.Parameters.AddWithValue(PARAM_ID, this.ID);
                    return (command.ExecuteNonQuery() == 1);
                }
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_FLAG_ZDJBXXZJ,dao.Connection))
                {
        ...
        public void  LoadFromReader(SQLiteDataReader reader)
        {
			...
			if (!reader.IsDBNull(1)) glysdm = reader.GetString(1);
        ...
        public IEntity Copy()
        {
            Zdjbxxzj target=new Zdjbxxzj();
            target.ID=0;
            target.Glysdm=this.Glysdm;
