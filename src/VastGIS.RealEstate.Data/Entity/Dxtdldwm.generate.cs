...
namespace VastGIS.RealEstate.Data.Entity
{
    ...
    public partial class Dxtdldwm:INotifyPropertyChanging, INotifyPropertyChanged,IDatabaseEntity,IBasemapPolygon
    {
        #region 表结构
	    ...
	    private const string SQL_INSERT_DXTDLDWM = "INSERT INTO DXTDLDWM (TC, CASSDM, FSXX1, FSXX2, YSDM, DatabaseId, FLAGS, geometry) VALUES ( @TC, @CASSDM, @FSXX1, @FSXX2, @YSDM, @DatabaseId, @FLAGS, GeomFromText(@geometry,@SRID));" + " SELECT last_insert_rowid();";
	
	    private const string SQL_UPDATE_DXTDLDWM = "UPDATE DXTDLDWM SET TC = @TC, CASSDM = @CASSDM, FSXX1 = @FSXX1, FSXX2 = @FSXX2, YSDM = @YSDM, DatabaseId = @DatabaseId, FLAGS = @FLAGS, geometry = GeomFromText(@geometry,@SRID) WHERE Id = @Id";
	
	    private const string SQL_DELETE_DXTDLDWM = "DELETE FROM DXTDLDWM WHERE  Id = @Id ";
        
        private const string SQL_DELETE_FLAG_DXTDLDWM = "UPDATE DXTDLDWM Set Flags=3 WHERE  Id = @Id ";
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_INSERT_DXTDLDWM,connection))
            {	
        ...
        {
            using(SQLiteCommand command  = new SQLiteCommand(SQL_UPDATE_DXTDLDWM,connection))
            {							
		...
		public bool Delete(SQLiteConnection connection)
        {
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_DXTDLDWM,connection))
                {
            ...
            {
                using(SQLiteCommand command  = new SQLiteCommand(SQL_DELETE_FLAG_DXTDLDWM,connection))
                {
