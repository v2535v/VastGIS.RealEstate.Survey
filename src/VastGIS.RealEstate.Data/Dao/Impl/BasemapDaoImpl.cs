...
namespace VastGIS.RealEstate.Data.Dao.Impl
{
    ...
    {
        private string CREATE_VIEW_BASEMAPD = "CREATE VIEW DXTBASEMAPDVIEW AS select Id,TC,CASSDM,FH,FHDX,XZJD,FSXX1,FSXX2,YSDM,DatabaseId,FLAGS,XGR,XGSJ,geometry from DXTBASEMAPD Where [FLAGS] < 3;";
        ...
        private string SELECT_DXTBASEMAPD = "select Id,TC,CASSDM,FH,FHDX,XZJD,FSXX1,FSXX2,YSDM,DatabaseId,FLAGS,XGR,XGSJ,geometry from DXTBASEMAPD Where [FLAGS] < 3";

        private string CREATE_VIEW_BASEMAPM = "CREATE VIEW DXTBASEMAPMVIEW AS select Id,TC,CASSDM,FSXX1,FSXX2,YSDM,DatabaseId,FLAGS,XGR,XGSJ,geometry from DXTBASEMAPM Where [FLAGS] < 3;";
        ...
        private string CREATE_UPDATE_TRIGGER_BASEMAPM = "CREATE TRIGGER [vw_upd_DXTBASEMAPMVIEW] INSTEAD OF UPDATE OF [Id], [TC], [CASSDM], [FSXX1], [FSXX2], [YSDM], [DatabaseId], [FLAGS],[XGR],[XGSJ], [geometry] ON [DXTBASEMAPMVIEW] BEGIN  Update [DXTBASEMAPM] SET [Id]=NEW.[Id], [TC]=NEW.[TC], [CASSDM]=NEW.[CASSDM], [FSXX1]=NEW.[FSXX1], [FSXX2]=NEW.[FSXX2], [YSDM]=NEW.[YSDM], [DatabaseId]=NEW.[DatabaseId], [FLAGS]=NEW.[FLAGS], [XGR]=NEW.[XGR],[XGSJ]=NEW.[XGSJ],[geometry]=NEW.[geometry] WHERE ROWID=OLD.ROWID;END";
        ...
        private string SELECT_DXTBASEMAPM = "select Id,TC,CASSDM,FSXX1,FSXX2,YSDM,DatabaseId,FLAGS,XGR,XGSJ,geometry from DXTBASEMAPM Where [FLAGS] < 3";

        private string CREATE_VIEW_BASEMAPX = "CREATE VIEW DXTBASEMAPXVIEW AS select Id,TC,CASSDM,FH,FHDX,FSXX1,FSXX2,YSDM,DatabaseId,FLAGS,XGR,XGSJ,geometry from DXTBASEMAPX Where [FLAGS] < 3;";
        ...
        private string SELECT_DXTBASEMAPX = "select Id,TC,CASSDM,FH,FHDX,FSXX1,FSXX2,YSDM,DatabaseId,FLAGS,XGR,XGSJ,geometry from DXTBASEMAPX Where [FLAGS] < 3";

        private string CREATE_VIEW_BASEMAPZJ = "CREATE VIEW DXTBASEMAPZJVIEW AS select Id,WBNR,TC,CASSDM,FH,FHDX,XZJD,YSDM,DatabaseId,FLAGS,XGR,XGSJ,geometry from DXTBASEMAPZJ Where [FLAGS] < 3;";

        private string CREATE_INSERT_TRIGGER_BASEMAPZJ = "CREATE TRIGGER [vw_ins_DXTBASEMAPZJVIEW] INSTEAD OF INSERT ON [DXTBASEMAPZJVIEW] BEGIN  INSERT OR REPLACE INTO [DXTBASEMAPZJ] ([Id], [WBNR], [TC], [CASSDM], [FH], [FHDX], [XZJD], [YSDM], [DatabaseId], [FLAGS],[XGR],[XGSJ], [geometry]) VALUES ( NEW.[Id], NEW.[WBNR], NEW.[TC], NEW.[CASSDM], NEW.[FH], NEW.[FHDX], NEW.[XZJD], NEW.[YSDM], NEW.[DatabaseId], NEW.[FLAGS], NEW.[XGR],NEW.[XGSJ],NEW.[geometry]); END";

        private string CREATE_UPDATE_TRIGGER_BASEMAPZJ = "CREATE TRIGGER [vw_upd_DXTBASEMAPZJVIEW] INSTEAD OF UPDATE OF [Id], [WBNR], [TC], [CASSDM], [FH], [FHDX], [XZJD], [YSDM], [DatabaseId], [FLAGS], [geometry] ON [DXTBASEMAPZJVIEW] BEGIN  Update [DXTBASEMAPZJ] SET [Id]=NEW.[Id], [WBNR]=NEW.[WBNR], [TC]=NEW.[TC], [CASSDM]=NEW.[CASSDM], [FH]=NEW.[FH], [FHDX]=NEW.[FHDX], [XZJD]=NEW.[XZJD], [YSDM]=NEW.[YSDM], [DatabaseId]=NEW.[DatabaseId], [FLAGS]=NEW.[FLAGS],[XGR]=NEW.[XGR],[XGSJ]=NEW.[XGSJ], [geometry]=NEW.[geometry] WHERE ROWID=OLD.ROWID;END";
        ...
        private string SELECT_DXTBASEMAPZJ = "select Id,WBNR,TC,CASSDM,FH,FHDX,XZJD,YSDM,DatabaseId,FLAGS,XGR,XGSJ,geometry from DXTBASEMAPZJ Where [FLAGS] < 3";



        string SELECT_BASEMAPD = "select Id,TC,CASSDM,FH,FHDX,XZJD,FSXX1,FSXX2,YSDM,DatabaseId,FLAGS,XGR,XGSJ,geometry from [DXT_LAYERNAME_D] Where [FLAGS] < 3";

        string SELECT_BASEMAPM = "select Id,TC,CASSDM,FSXX1,FSXX2,YSDM,DatabaseId,FLAGS,XGR,XGSJ,geometry from [DXT_LAYERNAME_M] Where [FLAGS] < 3";

        string SELECT_BASEMAPX = "select Id,TC,CASSDM,FH,FHDX,FSXX1,FSXX2,YSDM,DatabaseId,FLAGS,XGR,XGSJ,geometry from [DXT_LAYERNAME_X] Where [FLAGS] < 3";

        string SELECT_BASEMAPZJ = "select Id,WBNR,TC,CASSDM,FH,FHDX,XZJD,YSDM,DatabaseId,FLAGS,XGR,XGSJ,geometry from [DXT_LAYERNAME_ZJ] Where [FLAGS] < 3";
            
        string CREATE_BASEMAPD =
            "CREATE TABLE[DXT_LAYERNAME_D] ([Id] INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, [TC] NCHAR(100),[CASSDM] CHAR(20),[FH] NCHAR(30),[FHDX] FLOAT,[XZJD] FLOAT,[FSXX1] NCHAR(100),[FSXX2] NCHAR(100),[YSDM] CHAR(10),[DatabaseId] INTEGER DEFAULT 0,[FLAGS] SMALLINT DEFAULT 1,[XGR] VCHAR(10),[XGSJ] DATETIME);";

        string CREATE_BASEMAPM = "CREATE TABLE[DXT_LAYERNAME_M] ([Id] INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, [TC] NCHAR(100),[CASSDM] CHAR(20),[FSXX1] NCHAR(100),[FSXX2] NCHAR(100),[YSDM] CHAR(10),[DatabaseId] INTEGER DEFAULT 0,[FLAGS] SMALLINT DEFAULT 1,[XGR] VCHAR(10),[XGSJ] DATETIME);";

        string CREATE_BASEMAPX = "CREATE TABLE[DXT_LAYERNAME_X] ([Id] INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, [TC] NCHAR(100),[CASSDM] CHAR(20),[FH] NCHAR(30),[FHDX] FLOAT,[FSXX1] NCHAR(100),[FSXX2] NCHAR(100),[YSDM] CHAR(10),[DatabaseId] INTEGER DEFAULT 0,[FLAGS] SMALLINT DEFAULT 1,[XGR] VCHAR(10),[XGSJ] DATETIME);";

        string CREATE_BASEMAPZJ =
            "CREATE TABLE[DXT_LAYERNAME_ZJ] ([Id] INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, [WBNR] NCHAR(200),[TC] NCHAR(100),[CASSDM] CHAR(20),[FH] NCHAR(30),[FHDX] FLOAT,[XZJD] FLOAT,[YSDM] CHAR(10),[DatabaseId] INTEGER DEFAULT 0,[FLAGS] SMALLINT DEFAULT 1,[XGR] VCHAR(10),[XGSJ] DATETIME);";
            ...
            using (SQLiteTransaction trans = connection.BeginTransaction())
            {
                ...
                using (SQLiteCommand command = new SQLiteCommand(connection))
                {
                    ...
                    for (int i= CREATE_BASEMAP_LAYERS.Length-1; i>=0;i--)
                    {
                        ...
                        command.CommandText = GetRegisterClassSql("Basemap", tbName + "D", "IBasemapPoint");
                        command.ExecuteNonQuery();
                        command.CommandText = GetRegisterClassSql("Basemap", tbName + "X", "IBasemapPolyline");
                        command.ExecuteNonQuery();
                        command.CommandText = GetRegisterClassSql("Basemap", tbName + "M", "IBasemapPolygon");
                        command.ExecuteNonQuery();
                        command.CommandText = GetRegisterClassSql("Basemap", tbName + "ZJ", "IBasemapText");
                        command.ExecuteNonQuery();
        ...
        //{
        //    string sql = string.Format("SELECT ID, AsText(geometry) as Wkt,DatabaseID,Flags,Xgr,Xgsj,'{0}' As TableName,YSDM  from {0}", tableName);
        //    IList<IDatabaseFeature> features = connection.Query<IDatabaseFeature>(sql).ToList();
        //    int srid = GetSRID();
        //    using (SQLiteTransaction trans=connection.BeginTransaction())
        //    {
        //        foreach (IDatabaseFeature feature in features)
        //        {
        ...
        public bool AssignTextToAttribute()
        {
            ...
            VgSetting setting = service.GetVgSettingByName(SettingKeyHelper.DiXingTuWenZiShiBie);
