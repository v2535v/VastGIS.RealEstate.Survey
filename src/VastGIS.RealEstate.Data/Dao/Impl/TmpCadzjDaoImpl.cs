...
namespace VastGIS.RealEstate.Data.Dao.Impl
{
    public class TmpCadzjDaoImpl : SQLiteDao, TmpCadzjDao
    {
        ...
        private void Update(TmpCadzj cadd)
        {
            ...
            string query = String.Format("Update TmpCadzj Set {0} Where Id={1}", fieldParameters.ToString(),
                cadd.Id);
            command.CommandText = query;
            command.ExecuteNonQuery();
        }
        ...
        {
            SQLiteCommand command = new SQLiteCommand(connection);
            command.CommandText = String.Format("Select Id,Handle,EntityType,AsText(Geometry),FileName from TmpCadzj where EntityType like '{0}'", query);
        ...
        {
            SQLiteCommand command = new SQLiteCommand(connection);
            command.CommandText = String.Format("Select Id,Handle,EntityType,AsText(Geometry),FileName from TmpCadzj where Id={0}", id);
            DataTable dt = ExecuteSql(command);
        ...
        {
            SQLiteCommand command = new SQLiteCommand(connection);
            command.CommandText = String.Format("Select Id,Handle,EntityType,AsText(Geometry),FileName from TmpCadzj where Handle='{0}'", handle);
            DataTable dt = ExecuteSql(command);
            return ProcessResult(dt)[0];
        }
