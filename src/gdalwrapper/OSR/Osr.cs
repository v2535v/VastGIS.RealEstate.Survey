namespace OSGeo.OSR
{
    ...
    public class Osr
    {
        ...
        public static CoordinateTransformation CreateCoordinateTransformation(SpatialReference src, SpatialReference dst)
        {
            ...
            CoordinateTransformation transformation = (cPtr == IntPtr.Zero) ? null : new CoordinateTransformation(cPtr, true, ThisOwn_true());
        ...
        {
            IntPtr projectionMethodParameterList = OsrPINVOKE.GetProjectionMethodParameterList(method, out username);
            int num = 0;
            if (projectionMethodParameterList != IntPtr.Zero)
            {
                while (Marshal.ReadIntPtr(projectionMethodParameterList, num * IntPtr.Size) != IntPtr.Zero)
                {
                    num++;
                }
            }
            ...
            if (num > 0)
            {
                for (int i = 0; i < num; i++)
                {
                    IntPtr ptr = Marshal.ReadIntPtr(projectionMethodParameterList, i * Marshal.SizeOf(typeof(IntPtr)));
                    strArray[i] = (ptr == IntPtr.Zero) ? null : Marshal.PtrToStringAnsi(ptr);
                }
            }
            if (projectionMethodParameterList != IntPtr.Zero)
            {
                OsrPINVOKE.StringListDestroy(projectionMethodParameterList);
            }
            if (OsrPINVOKE.SWIGPendingException.Pending)
            {
                throw OsrPINVOKE.SWIGPendingException.Retrieve();
        ...
        {
            IntPtr projectionMethods = OsrPINVOKE.GetProjectionMethods();
            int num = 0;
            if (projectionMethods != IntPtr.Zero)
            {
                while (Marshal.ReadIntPtr(projectionMethods, num * IntPtr.Size) != IntPtr.Zero)
                {
                    num++;
                }
            }
            ...
            if (num > 0)
            {
                for (int i = 0; i < num; i++)
                {
                    IntPtr ptr = Marshal.ReadIntPtr(projectionMethods, i * Marshal.SizeOf(typeof(IntPtr)));
                    strArray[i] = (ptr == IntPtr.Zero) ? null : Marshal.PtrToStringAnsi(ptr);
                }
            }
            if (projectionMethods != IntPtr.Zero)
            {
                OsrPINVOKE.StringListDestroy(projectionMethods);
            }
            if (OsrPINVOKE.SWIGPendingException.Pending)
            {
                throw OsrPINVOKE.SWIGPendingException.Retrieve();
        ...
        {
            int userInputAsWKT = OsrPINVOKE.GetUserInputAsWKT(name, out argout);
            if (OsrPINVOKE.SWIGPendingException.Pending)
            {
                throw OsrPINVOKE.SWIGPendingException.Retrieve();
            }
            return userInputAsWKT;
        }

        public static int GetWellKnownGeogCSAsWKT(string name, out string argout)
        {
            int wellKnownGeogCSAsWKT = OsrPINVOKE.GetWellKnownGeogCSAsWKT(name, out argout);
            if (OsrPINVOKE.SWIGPendingException.Pending)
            {
                throw OsrPINVOKE.SWIGPendingException.Retrieve();
            }
            return wellKnownGeogCSAsWKT;
        }
