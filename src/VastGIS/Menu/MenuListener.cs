        ...
        private bool HandleLayerMenu(string itemKey)
        {
            switch (itemKey)
            {
                ...
                case MenuKeys.AddDatabaseLayer:
                    ...
                    return true;
                case MenuKeys.CreateZDDatabase:
                    SaveFileDialog dialog=new SaveFileDialog();
                    dialog.Title = "数据库";
                    dialog.Filter = "Sqlite数据库(*.db)|*.db";
                    if (dialog.ShowDialog() != DialogResult.OK) return false;

                    SQLiteConnection.CreateFile(dialog.FileName);
                    SQLiteConnection connection2=new SQLiteConnection("Data Source="+dialog.FileName + "; Version = 3;");
                    connection2.Open();
                    var spatialitePath = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location),
                        (Environment.Is64BitProcess ? "x64" : "x86"), "spatialite");
                    string mod_spatialite_dllPath = spatialitePath + @"\mod_spatialite";
                    connection2.LoadExtension(mod_spatialite_dllPath);
                    SQLiteCommand command=new SQLiteCommand(@"select InitSpatialMetaData();",connection2);
                    command.ExecuteNonQuery();
                    //_bdcDatabaseService.CreateDatabase();
                    //BDCDatabaseService service=new BDCDatabaseService(_context);
